<?php 
include '../header.php'; ?>

<ul class="breadcrumb">
  <li><a href="<?php echo $siteurl; ?>">Home</a> <span class="divider">&rarr;</span></li>
  <li class="active">2013 Junior Olympics</li>
</ul>

<div class="row">
	<div class="span12">
		<h2>Choose from an age group/division to continue</h2>
		<h3 class="lead">Live streaming video and box score can be found by going to the Schedule and then clicking "Go To Game" for the game you'd like to view.</h3>
	</div>
</div>

<div class="divisions">
	
	<div class="row">
		<div class="span4">
			<h4>6U - ALL</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>

		<div class="span4">
			<h4>8U - A</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>8U - AA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>8U - AAA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>

		<div class="span4">
			<h4>10U - A</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>8U - AA/AAA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>
	</div><!-- .row -->

	<div class="row">
		<div class="span4">
			<h4>12U - ALL</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>

		<div class="span4">
			<h4>14U - A</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>14U - AA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>14U - AAA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>

		<div class="span4">
			<h4>16U - A</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>16U - AA/AAA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>
	</div><!-- .row -->

	<div class="row">
		<div class="span4">
			<h4>18U - ALL</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>

		<div class="span4">
			<h4>21U - A</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>21U - AA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
			<h4>21U - AAA</h4>
			<div class="btn-group">
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/schedule">Schedule</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/stats">Stats</a>
				<a class="btn btn-success" href="<?php echo $siteurl; ?>/tourney/standings">Standings</a>
			</div>
		</div>
	</div><!-- .row -->
</div>

<?php include '../footer.php'; ?>